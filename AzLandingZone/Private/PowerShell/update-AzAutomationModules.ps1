function Update-AzAutomationModules {
    ...
    function Import-AutomationModule([string] $ModuleName, [bool] $UseAzModule = $false) {
        ...
        do {
            $ModuleContentUrl = (Invoke-WebRequest -Uri $ModuleContentUrl -MaximumRedirection 0 -UseBasicParsing -ErrorAction Ignore -SkipHttpErrorCheck).Headers.Location 
        } while (!($ModuleContentUrl -like "*.nupkg"))
        ...
        if ($CurrentModule.Version -eq $LatestModuleVersionOnGallery) {
            ...
            & $NewAutomationModule `
                ...
                -AutomationAccountName $AutomationAccountName `
                -Name $ModuleName `
                -ContentLink $ModuleContentUrl[0] > $null
        }
    }
    ...
    function Update-ProfileAndAutomationVersionToLatest([string] $AutomationModuleName) {
        ...
        $ProfileURL = (Invoke-WebRequest -Uri $ModuleContentUrl -MaximumRedirection 0 -UseBasicParsing -ErrorAction Ignore -SkipHttpErrorCheck).Headers.Location
        ...
        $AutomationURL = (Invoke-WebRequest -Uri $ModuleContentUrl -MaximumRedirection 0 -UseBasicParsing -ErrorAction Ignore -SkipHttpErrorCheck).Headers.Location
