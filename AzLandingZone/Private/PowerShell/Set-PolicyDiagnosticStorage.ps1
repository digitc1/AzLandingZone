Function Set-PolicyDiagnosticStorage {
    ...
    $definitionListURI = "https://raw.githubusercontent.com/digitc1/AzLandingZonePublic/master/definitions/definitionList1.txt"
    ...
    Get-Content -Path $HOME/definitionList.txt | ForEach-Object -Parallel{
        ...
        $GetStorageAccount = $using:GetStorageAccount
    ...
    if ($policyInitiative = Get-AzPolicySetDefinition -ManagementGroupName $GetManagementGroup.Name | where-Object { $_.Name -Like "SLZ-policyGroup1" }) {
        if(!$policyInitiative.Properties.PolicyDefinitions.Parameters.region){
            ...
            Remove-AzPolicySetDefinition -ManagementGroupName $GetManagementGroup.Name -Name "SLZ-policyGroup1"
...
Export-ModuleMember -Function Set-PolicyDiagnosticStorage
