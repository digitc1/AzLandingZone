Function Set-LzSentinelAnalyticsRule {
    ...
    try{
        ...
        ($requestResult.Content | ConvertFrom-Json).value | ForEach-Object {
            if($_.kind -Like "Scheduled"){
                $body = @{
                    ...
                    'properties' = @{
                        ...
                        'alertRuleTemplateName' = ($_.Id -split '/')[-1]
                        'enabled' = 'true'
                    }
                }
            }
            ...
            $uri = "https://management.azure.com" + $GetLogAnalyticsWorkspace.ResourceId  + "/providers/Microsoft.SecurityInsights/alertRules/" + ($_.Id -split '/')[-1] + "?api-version=2020-01-01"
        
            Write-Host -ForegroundColor Green "Enabling analytics rule $_.Id"
            $auth = Get-LzAccessToken
            $requestResult = Invoke-webrequest -Uri $uri -Method Put -Headers $auth -Body ($body | ConvertTo-Json -Depth 5)
            Write-Host -ForegroundColor Green "Enabled analytics rule $_.Id"
        }
    }
    catch {
